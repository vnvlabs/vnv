...
/*
 *  This test writes a distributed vector to file. It requires a std::vector<double>
 *  x to run. The input file allows you to map injection point parameters
 *  to test parameters.
 *
...
INJECTION_TEST(SPNAME, write_vec, std::vector<double> x) {
    ...
    // Write a vector indexed by the rank with local size x.size() on each process.
    engine->Put_Vector(comm, "Vector1", x );

    //Write a vector using the raw data. In this case we set the local size
    // to 2. The final vector written will be world_size * 2
    engine->Put_Vector(comm, "Vector2", x.size(), x.data());

    // Write using reference to a single element
    engine->Put_Vector(comm, "Vector3", x[0]);
    return SUCCESS;
 ...
 *
 * template<typename T>
 * auto& x() { return getReference<std::vector<double> x>("std::vector<double> x");
 */
INJECTION_TEST(SPNAME, put_rank, std::vector<double> x) {
    ...
    // Write a vector indexed by the rank with local size x.size() on each process.
    engine->Put_Rank(comm, "Vector1", x, rank );

    //Write a vector using the raw data. In this case we set the local size
    engine->Put_Rank(comm, "Vector2", x.size(), x.data(), rank);

    // Write using reference to a single element
    engine->Put_Rank(comm, "Vector3", x[0], rank);
...
INJECTION_TEST(SPNAME, put_matrix, std::vector<std::vector<double>> x) {
    ...
    engine->Put_Matrix(comm, "Matrix1", x, x[0].size() );
    ...
    engine->Put_Matrix(comm, "Matrix2", x, gsize, offs );
    return SUCCESS;
...
INJECTION_TEST(SPNAME, put_reduce, std::vector<double> x) {
    ...
    engine->Put_ReduceVector(comm, "Reduce1", "VNV:max", x);

    // Reduce a scalar across all process. This finds the max of x[1]
    // across all processes.
    engine->Put_ReduceScalar(comm, "Reduce2", "VNV:max", x[1]);

    // Reduce the vector element wise == produces a vector of size x.size()
    engine->Put_ReduceVectorElementWise(comm, "Reduce3", "VNV:sum", x);

    // Reduce the vector on every process and write a vector of length comm_size
    engine->Put_ReduceVectorRankWise(comm, "Reduce4", "VNV:sum", x );

    // Reduce the vector on rank 0 and use that as the result
    engine->Put_ReduceVectorRankOnly(comm, "Reduce5", "VNV:sum", x , 0);

    // Run a custom reduction on a global vector
    engine->Put_ReduceVector(comm,"Reduce6","Distributed:custom_reduction", x );
    return SUCCESS;
...
void cust(void* in, void* inout, int* len, MPI_Datatype* data) {
    ...
    double* d = (double*) inout;
    double* di = (double*) inout;
    d[0] += di[0];
    d[1] += di[1];
...
int main(int argc, char** argv) {
  ...
  /**
    ...
    * ------------------------
    *
    * This executable demonstrated the distributed data capabilities of the IOutputEngine
    * interface. The example provides a single injection point with a vector of doubles as
    * the only available input parameter.
    *
    * In this example, we provide several VnV tests demoing the distributed data support.
    * These test expect a vector of doubles as input and write data to using the various
    * distributed data functions.
    *
    * The example expects a file called "vv-dist-data.json"  to be available in the directory
    * it is run from.
    *
    * The executable expects two command line arguements. The first represents the local size of
    * the vector on each process. The second parameter represents the size of the matrix on each
    * local process for the matrix examples.
    *
    * For example mpirun -n p <exe> 10 3 will create a vector of length p*size and a matrix of size [3*p,3*p]
    *
    *
    * .. hint::
    *    This comment will show up in the final report as the introduction. It supports
   ...
   // so each process owns the range [rank*local_vec_size, rank*(local_vec_size+1))
   int local_vec_size = std::atoi(argv[1]);
   int global_vec_size = size*local_vec_size;
    ...
    *
    */
   INJECTION_POINT(SPNAME,VWORLD(SPNAME), distribute, local_vec, local_mat)
