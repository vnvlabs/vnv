...
class SampleAction : public VnV::IAction {
 ...
 public:
  SampleAction(const nlohmann::json& config) { this->config = config; }

  virtual void initialize() { getEngine()->Put("config", config); }
};
...
INJECTION_ACTION(VnVPlugin, sampleAction, R"({"type":"object"})") { return new SampleAction(config); }
