...
// The toolkit has a built in unit testing framework. Tests can have any number
// of processors involved in them. The last parameter to the INJECTION_UNITTEST
// macro is the number of cores VnV will only run tests where we have enough
// cores to do so. When running unit tests, VnV creates custom sub-communicators
// to execute the tests. A communication packing algorithm is used to
// parallelize the execution of unit tests where ever possible.
