...
INJECTION_TEST(PNAME, customTest, double x) {
   ...
   // You can pull parameters out of the test parameter map using the get function. 
   const double& x = get<double>("x");

   // You can write values to the output engine using the IOutputEngine interface. These
   // values can be used in the test comment template as shown above. 
   engine->Put("x",x);
   return SUCCESS;
}
...
// You can also define a simple runner class. This allows you to organize your tests 
// using OOP principles. The only requirement for a runner is that it has a public 
// constructor with no parameters. When a runner is defined, you will have access 
// to a shared_ptr called "runner" pointing to an instance of the runner class (std::shared_ptr<CRunner>)  
class CRunner {
public:
    CRunner() {}

    void run(VnV::IOutputEngine* engine) {
        engine->Put("x",10);
    }    
};
...
INJECTION_TEST_RS(PNAME, customTest3, CRunner, c3Schema, double x) {
    ...
    // Calling getConfigurationJson() returns a const reference to the user provided
    // json configuration object that has been validated against the schema provided 
    // eariler. With a good schema, you can avoid checking the existence and type of 
    // parameters. 
 ...
 */
 INJECTION_TEST(PNAME, customTest4, double y) {
     ...
     // Injection points have stages. You can get the current stage using the type variable
     switch (type)
     {
     ...
     case InjectionPointType::Iter:
         ...
         // We can also provide meta data to any Put. Metadata is a string->string map
         // of values that will be associated with the variable.    
