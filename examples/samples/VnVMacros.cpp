...
int main(int argc, char** argv) {
   ...
   // Logging -- Default it to only write logs on the root comm of the world processor.
    ...
    *
    *   1. The Package name. This is the name of the package that places the injection
    *      point. In this case, we called our package "ADD_INJECTION_POINT".
    *   2. The Communicator. The communicator defines the group of processors that will
    *      call this injection point. You can call VSELF, VWORLD or VMPI(MPI_Comm). If this 
    *      is a serial application you should use VSELF. The injection point is collective 
    *      across this communicator. 
    *   3. The Injection Point name. The name of the injection point is used to identify 
    *      the injection point in reports and input files. It should be a string literal 
    *      that is unique within the package.  
    *   4. The parameter list. The parameter list is a list of parameters that you want to
    *      make available at the injection point. Injection tests will be able to access these
    *      parameters for analysis purposes. In this case, we pass the parameter argc. 
    *   5. The comment. The comment is a c++ comment block located directly above the injection
    *      point. The comment is a Restructured text template that will be rendered with each instance
    *      of the injection point in the final report. That is to say, this text will appear in the final
    *      report. 
    */ 
    ...
    * 
    * */ 
    INJECTION_POINT_C("Samples", VSELF, "MY_FIRST_INJECTION_POINT_CALLBACK", IPCALLBACK {
    ...
    * 
    * Loops let you monitor a region of code. Variables must be valid scope across the entire loop.
    * All tests will have a lifetime equal to the lifetime of the loop. Loops should have a begining 
    * and and end statement, any number of ITER statements are aloud. 
    */ 
   int i = 0;
 ...
 /**
    ...
    * 
    */ 
   INJECTION_ITERATION(b, "Samples", VSELF, "equal", 1, 1, x, z ) {
        z = x;
   }
