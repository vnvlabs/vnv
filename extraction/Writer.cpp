...
contains ! Implementation of the functions. We just wrap the C function here.
  ...
  if (ofs.good()) {
    ofs << std::regex_replace(fortranModuleTemplate,std::regex("####"),package.c_str());
  }
  ofs.close();
}
      ...
      for (auto it : j["Options"].items()) {
        ...
        if (packageName.empty() || pname == packageName) {
          ...
          if (it.value().contains("schema")){
            oss_register[pname] << "\tREGISTERRAWOPTIONS(" << pname << "," << VnV::StringUtils::escapeQuotes(it.value()["schema"].dump(), true) << ")\n";
          } else {
