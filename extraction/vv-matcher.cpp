...
namespace {
  ...
  void jsonAdder(std::string packageName, json& input, json& output, std::string key) {
  ...
  //Copied shamelessly from an anonomous namespace in the Clang source code in RawCommentList.cpp
  ...
  std::map<std::string,std::string> parseArgs(Preprocessor &pp, const Token *token){
  ...
  unsigned int  getInfo(const CallExpr *call, const FunctionDecl *func, const MatchFinder::MatchResult &Result , json &info, std::string &id, std::string &filename, int begin)
  {
...
public:
    ...
    if (!packageName.empty()) {
       ...
       if (it != oss_declare.end()) {
          return it->second.str();
            ...
            // This means the package includes VnV.h, but does not have any VnV INJECTION CALLS.
            // So, we should return a empty configuration.
  ...
  RegistrationWriter(json& j, std::string packageName="") {
    ...
    //Catch the options. They are a little different from above (no name parameter).
    ...
    if (j.contains("InjectionPoints")) {
        for (auto it : j["InjectionPoints"].items()) {
           ...
           if (packageName.empty() || pname == packageName) {
               ...
               oss_register[pname] << "\tRegister_Injection_Point(" << name << "," << escaped << ");\n";
           }
       }
    }
    finalizeAllPackages();
...
namespace {
  ...
  public:
    ...
    PreprocessCallback(std::string packageName, json &j, std::set<std::string> &includes, Preprocessor &PP) : PPCallbacks(), pp(PP), thisJson(j), modTime(includes) {
    ...
    void MacroExpands(const Token &MacroNameTok, const MacroDefinition &MD, SourceRange Range, const MacroArgs *Args) override {
       ...
       if (nae == "INJECTION_TEST_R") {
           ...
           json &stages = getOrCreate(jj,"stages");
           json &thisStage = getOrCreate(stages,pp.getSpelling(*Args->getUnexpArgument(1)));
           thisStage["docs"] = getDocs(Range);
    ...
    void MacroDefined(const Token &MacroNameTok, const MacroDirective *MD) override {
      ...
      if (nae == "PACKAGENAME") {
          std::string declares = "";
          for (const Token &it : MD->getDefinition().getDirective()->getMacroInfo()->tokens()) {
          ...
          if (packName.empty() || VnV::StringUtils::trim_copy(declares) == packName ) {
            pp.addCommentHandler(this);
            active = true;
          } else {
          //TODO -- Technically, we can abort here. Not sure if possible using the
          //pp callbacks API.
         }
      }
    }
  ...
  public:
    ...
    void ExecuteAction() {
      ...
      std::time_t lastMod = SRC.getFileManager().getFile(filename)->getModificationTime();
      subJson = json::object();
      PP.addPPCallbacks(
  ...
  public:
    VnVPackageFinderFrontendActionFactory(json &processed,std::string pname) : packageName(pname), mainJson(processed) {}
  ...
  public:
    ...
    VnVFinder() {
      ...
      StatementMatcher functionMatcher = callExpr(hasAncestor(functionDecl().bind("function")),
                                                  callee(functionDecl(hasName("_VnV_injectionPoint")))).bind("callsite");
      StatementMatcher functionMatcher1 = callExpr(hasAncestor(functionDecl().bind("function")),
                                                   callee(functionDecl(hasName("_VnV_injectionPoint_begin")))).bind("callsite_begin");
      StatementMatcher functionMatcher2 = callExpr(hasAncestor(functionDecl().bind("function")),
                                                   callee(functionDecl(hasName("_VnV_injectionPoint_loop")))).bind("callsite_iter");
      StatementMatcher functionMatcher3 = callExpr(hasAncestor(functionDecl().bind("function")),
                                                   callee(functionDecl(hasName("_VnV_injectionPoint_end")))).bind("callsite_end");

     //Cpp Matchers
     StatementMatcher functionMatcherC = callExpr(  hasAncestor(functionDecl().bind("function")),
                                                    callee(functionDecl(hasName("VnV::CppInjection::BeginPack")))).bind("cpp_callsite");
     StatementMatcher functionMatcher1C = callExpr(hasAncestor(functionDecl().bind("function")),
                                                   callee(functionDecl(hasName("VnV::CppInjection::BeginLoopPack")))).bind("cpp_callsite_begin");
     StatementMatcher functionMatcher2C = callExpr(hasAncestor(functionDecl().bind("function")),
                                                   callee(functionDecl(hasName("VnV::CppInjection::IterLoop")))).bind("callsite_iter");
     StatementMatcher functionMatcher3C = callExpr(hasAncestor(functionDecl().bind("function")),
                                                   callee(functionDecl(hasName("VnV::CppInjection::EndLoop")))).bind("callsite_end");
...
void writeFileAndCache(json &cacheInfo, std::string outputFileName, std::string cacheFile, std::string packageName, bool writeFile) {
  //HEREEEEEE
  // Convert from the file->Objects cache data structure to a ObjectType -> format.
  ...
  if (writeFile) {
      ...
      for (auto it : cacheInfo["data"].items()) {
        ...
        for (std::string type : {"InjectionPoints", "SubPackages", "LogLevels", "Tests", "Engines", "Serializers", "Transforms", "UnitTests" } ) {
      ...
      oss << "/// This file was automatically generated using the VnV-Matcher executable. \n"
...
int main(int argc, const char **argv) {
  ...
  if (hasCache) {
     ...
     for (auto &it : files) { // all files to be compiled. (strings)
       ...
       if (cacheFiles.contains(it)) {
          for (auto f : cacheFiles[it].items()) { // list of ids included in this file.
     ...
     if ( modFiles.size() == 0 ) {
         if ( outputFileName != cacheInfo[LAST_FILE_NAME] || lastRunTime < timeForFile(outputFileName) ) {
  ...
  // Get a list of files that contain Injection points with the current package Name.
  ...
  for (auto it : vnvDeclares1.items()) {
      ...
      if ((packageName_.empty() || ( data.contains("packageName") && data["packageName"].get<std::string>() == packageName_) )) {
  ...
  //Search the AST to extract information about injection points. In particular,
  ...
  for ( auto fil : Finder.get().items() ) {
      ...
      for (auto it : fil.value().items()) {
          json &ip = getOrCreate(injectData,it.key()); // the data for this injection point.
          json &val = it.value();

          ip["parameters"] = (val.contains("parameters") ? it.value()["parameters"] : json::array());
  ...
  //Write the file and cache. Always update the cache -- to get the updated times, but only write the file if
  // the filename has changed or the actual data has changed.
  bool writeFile = ( (cacheInfo[LAST_FILE_NAME] != outputFileName ) ||  hasher(cacheData.dump()) != cacheDataHash );
  writeFileAndCache(cacheInfo, outputFileName, cacheFile_, packageName_,writeFile);
  return 0;
}
