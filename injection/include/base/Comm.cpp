...
class DataTypeCommunication {
    ...
    std::pair<std::vector<std::pair<IStatus_ptr, int>>,int> TestAny(IRequest_vec &vec) {
    ...
    IDataType_vec BroadCast(IDataType_vec &data, int count, int root, bool allToAll ) {
        int rank = comm->Rank();
        int size = comm->Size();
    ...
    IDataType_vec Reduce(IDataType_vec &data, IReduction_ptr reduction, int root ) {
        ...
        long long * recvBuffer = (root < 0 || rank==root ) ? (long long*) malloc(buffSize) : nullptr;
        if (root == -1 ) {
