...
typedef ICommunicator* (*comm_factory)(CommType); // Empty comm to be populated with a comm later.
...
class CommunicationStore {
 ...
 private:
  ...
  // keys across processors in buffers. Long long avoids sending the char arrays.
  ...
  void addCommunicator(std::string packageName, std::string name, comm_factory factory);
  ...
  ICommunicator_ptr getCommunicator(std::string packageName, std::string name, CommType type);
