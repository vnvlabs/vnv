...
class IteratorConfig : public TestConfig {
 public:
  IteratorConfig(std::string package, std::string name, json& userConfig,
                 std::map<std::string, std::string>& params)
      : TestConfig(package, name, userConfig, params) {}
};
...
void registerIterator(std::string package, std::string name, std::string schema,
                      VnV::iterator_maker_ptr m,
...
#define INJECTION_ITERATOR_RS(PNAME, name, Runner, schema, ...)                \
  ...
  class name : public Test_T<VnV_Arg_Type(Runner), VnV::IIterator> {           \
   public:                                                                     \
    name(IteratorConfig& config)                                               \
        : Test_T<VnV_Arg_Type(Runner), VnV::IIterator>(config, #__VA_ARGS__) { \
    }                                                                          \
    int iterate(ICommunicator_ptr comm, IOutputEngine* engine) override;       \
  };                                                                           \
  IIterator* declare_##name(IteratorConfig config) {                           \
    return new name(config);                                                   \
  }                                                                            \
  void register_##name() {                                                     \
    VnV::registerIterator(#PNAME, #name, schema, &declare_##name,              \
                          VnV::StringUtils::variadicProcess(#__VA_ARGS__));    \
  }                                                                            \
  }                                                                            \
  }                                                                            \
  }                                                                            \
  int VnV::PNAME::Iterators::name::iterate(ICommunicator_ptr comm,             \
...
#define INJECTION_ITERATOR_R(PNAME, name, runner, ...)               \
  INJECTION_ITERATOR_RS(PNAME, name, runner, R"({"type":"object"})", \
                        __VA_ARGS__)
