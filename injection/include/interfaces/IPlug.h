...
void registerPlug(std::string package, std::string name, std::string schema,
                  VnV::plug_maker_ptr m,
...
#define INJECTION_PLUG_RS(PNAME, name, Runner, schema, ...)                 \
  ...
  void register_##name() {                                                  \
    VnV::registerPlug(#PNAME, #name, schema, &declare_##name,               \
                      VnV::StringUtils::variadicProcess(#__VA_ARGS__));     \
  }                                                                         \
  }                                                                         \
  }                                                                         \
  }                                                                         \
  bool VnV::PNAME::Plugs::name::plug(ICommunicator_ptr comm,                \
