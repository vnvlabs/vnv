...
void registerVnVSampler(std::string packageName, std::string name, std::string schema, VnV::sampler_ptr r);
...
template <typename Runner> class Sampler_T : public ISampler {
 public:
  ...
  Sampler_T(nlohmann::json& config_) : ISampler(), config(config_), runner(new Runner()) {}
};
...
#define INJECTION_SAMPLER_RS(PNAME, name, Runner, schema)                                             \
  ...
  void register_##name() { VnV::registerVnVSampler(VNV_STR(PNAME), #name, schema, &declare_##name); } \
  }                                                                                                   \
  }                                                                                                   \
  }                                                                                                   \
...
#define INJECTION_SAMPLER_R(PNAME, name, Runner) INJECTION_SAMPLER_RS(PNAME, name, Runner, R"({"type":"object"})")

#define INJECTION_SAMPLER(PNAME, name) INJECTION_SAMPLER_S(PNAME, name, R"({"type":"object"})")
