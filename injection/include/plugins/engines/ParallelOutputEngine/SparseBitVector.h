...
#ifndef _SPARSEBITVECTOR_H_
#define _SPARSEBITVECTOR_H_
...
class SparseBitVectorElement {
 ...
 public:
  ...
  MSGPACK_DEFINE(ElementIndex, Bits);
};

class SparseBitVector {
  using ElementList = std::list<SparseBitVectorElement>;
  using ElementListIter = typename ElementList::iterator;
  using ElementListConstIter = typename ElementList::const_iterator;
  enum { BITWORD_SIZE = SparseBitVectorElement::BITWORD_SIZE };
  ...
  ElementListIter FindLowerBoundImpl(unsigned ElementIndex) const {
    ...
    ElementListIter Begin = const_cast<SparseBitVector*>(this)->Elements.begin();
    ElementListIter End = const_cast<SparseBitVector*>(this)->Elements.end();
  ...
  class SparseBitVectorIterator {
   private:
    ...
    const SparseBitVector* BitVector = nullptr;
   ...
   public:
    ...
    SparseBitVectorIterator(const SparseBitVector* RHS, bool end = false) : BitVector(RHS) {
      Iter = BitVector->Elements.begin();
      BitNumber = 0;
      Bits = 0;
      WordNumber = ~0;
 ...
 public:
  ...
  SparseBitVector(int idx) : Elements(), CurrElementIter(Elements.begin()) { set(idx); }
  ...
  void intersectWithComplement(const SparseBitVector& RHS1, const SparseBitVector& RHS2) {
    if (this == &RHS1) {
  ...
  bool intersects(const SparseBitVector* RHS) const { return intersects(*RHS); }
  ...
  int find_last() const {
    if (Elements.empty()) return -1;
    const SparseBitVectorElement& Last = *(Elements.rbegin());
  ...
  std::string toString() {
    bool in_range = true;
    int begin_range = *begin();
    int end_range = begin_range;
    std::stringstream ss;
    ss << "[";
    if (begin() != end()) {
      for (iterator it = begin(); it != end(); ++it) {
        if (*it > end_range + 1) {
          ss << begin_range;
          if (begin_range != end_range) ss << "-" << end_range;
          ss << ",";
          begin_range = *it;
        }
        end_range = *it;
...
template <unsigned ElementSize> inline bool operator|=(SparseBitVector& LHS, const SparseBitVector* RHS) {
  return LHS |= *RHS;
}
...
inline SparseBitVector operator|(const SparseBitVector& LHS, const SparseBitVector& RHS) {
