...
class VnVTreeGenerator:
     ...
     def visitTests(self, outputDirectory, node, jmespath, userArgs):
          ...
          for child in node:
               ...
               jmes = jmespath + "[?Id == '{}']".format(child.getId())
     ...
     def visitDocumentationNode(self,outputDirectory, node, jmespath, userArgs):
          ...
          if template is None:
               ...
               if darray is not None:
                    ...
                    for d in node.getData():
                         jmes = ".[?Id == '{}']".format(d.getId())
                         data.append("Name: {}, Type: {}".format(d.getId(),VnVReader.type2Str[d.getType()]))
     ...
     def visitUnitTests(self, outputDirectory, node, jmespath, userArgs):
          ...
          for package in packages:
               ...
               for test in packages[package]:
                    ...
                    pjmes = jmespath + "[?id=='{}']".format(test.getId())
     ...
     def generateTree(self, outputDirectory, rootNode):
          ...
          # Use sphinx to create a quickstart if requested.
          if "quickstart" in self.jsonInfo:
               opts = self.jsonInfo.get("quickstart")
               path = opts.get("path","sphinx-quickstart")
               options = opts.get("options",[])
               run = [path] + options + [self.dir]
               x = subprocess.run(run)
               print("Sphinx-Quick start returned {}".format(x.returncode))
               # Override the sphinx starter template with our own one.
               with open(os.path.join(self.dir,"index.rst"),"w") as f:
                    date = datetime.datetime.today()
                    datet = date.strftime("%a %b %y")
                    fname = self.jsonInfo.get("filename","index")
                    fname = os.path.join("vnv",fname)
                    t = VnVTreeGeneratorSchema.vnvStarterTemplate
                    f.write(t.format(date=datet, rootNode=fname))

               self.dir = os.path.join(self.dir,"vnv")
          ...
          tocTree.append(self.ftt("rootNode",self.visitRootNode(outDir, rootNode, "", self.jsonInfo.get("rootNode",{}))))
          ...
          stream.write(RestUtils.tocTree(tocTree))
          ret = RestUtils.writeFile(self.dir, self.jsonInfo.get("filename","index"),stream.getvalue())

          if self.jsonInfo.get("build",False):
               # Build the thing at the same time.
               options = self.jsonInfo.get("build_opts",["html"])
               subprocess.run(["make"] + options, cwd=outputDirectory)
