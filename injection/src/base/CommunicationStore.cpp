...
long long CommunicationStore::getKey(std::string packageName, std::string name) {
...
void CommunicationStore::addCommunicator(std::string packageName, std::string name, comm_factory m) {
     communicator_factory.insert(std::make_pair(getKey(packageName,name),m));
}



void CommunicationStore::addReduction(std::string packageName, std::string name, reduction_ptr m) {
     reduction_factory.insert(std::make_pair(getKey(packageName,name),m));
}



void CommunicationStore::addDataType(std::string packageName, std::string name, datatype_ptr m) {
...
IDataType_ptr CommunicationStore::getDataType(std::string packageName, std::string name) {
...
IReduction_ptr CommunicationStore::getReducer(std::string packageName, std::string name) {
     return getReducer(getKey(packageName,name));
}



ICommunicator_ptr CommunicationStore::getCommunicator(std::string packageName, std::string name, CommType type) {
     return getCommunicator(getKey(packageName, name), type);
}



ICommunicator_ptr CommunicationStore::getCommunicator(long long key, CommType type) {
