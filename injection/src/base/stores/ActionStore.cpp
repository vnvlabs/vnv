...
void ActionStore::addAction(std::string packageName, std::string name,
                            action_ptr m) {
...
nlohmann::json ActionStore::schema() {
  ...
  for (auto& it : action_factory) {
    ...
    for (auto& itt : it.second) {
      props1[itt.first] =
          R"({"type" : "string" , "enum" : ["off", "start","end","both"] })"_json;
    }
...
IAction* ActionStore::getAction(std::string packageName, std::string name) {
...
void ActionStore::runAction(ICommunicator_ptr comm, std::string packageName,
  ...
  if (action->setConfig(config)) {
    ...
    VnV_Warn(VNVPACKAGENAME, "Invalid configuration for action %s:%s",
             packageName.c_str(), name.c_str());
  }
}
