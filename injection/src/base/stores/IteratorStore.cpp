...
std::shared_ptr<IterationPoint> IteratorStore::newIterator(std::string packageName, std::string name, int once_, NTV& in_args, NTV& out_args) {
  ...
  if (it != iterators.end() && reg != registeredIterators.end()) {
    ...
    injectionPoint.reset( new IterationPoint(packageName, name, reg->second.specJson, once_, in_args, out_args));
...
void IteratorStore::registerIterator(std::string packageName,
                                                 std::string id,
                                                 json& jsonObject) {
...
void IteratorStore::addIterator(std::string package,
  ...
  auto reg = registeredIterators.find(key);
  json parameters = (reg == registeredIterators.end()) ? json::object() : (reg->second.specJson);
