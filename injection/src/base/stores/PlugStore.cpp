...
std::shared_ptr<PlugPoint> PlugStore::newPlug(std::string packageName, std::string name, NTV& in_args, NTV& out_args) {
  ...
  if (it != plugs.end() && reg != registeredPlugs.end()) {
    ...
    plug.reset( new PlugPoint(packageName, name, reg->second.specJson, in_args, out_args));
...
std::shared_ptr<PlugPoint> PlugStore::getNewPlug(
    std::string package, std::string name, NTV& in_args, NTV& out_args) {
...
void PlugStore::addPlug(std::string package,
  ...
  auto reg = registeredPlugs.find(key);
  json parameters = (reg == registeredPlugs.end()) ? json::object() : (reg->second.specJson);
  ...
  if (nPlugs != nullptr) { 
      if (nPlugs->preLoadParameterSet(parameterMap)) {
           VnV_Debug(VNVPACKAGENAME, "Plug Added Successfully %s", nPlugs->getName().c_str());
      } else {
           VnV_Debug(VNVPACKAGENAME, "Plug Load failed %s", nPlugs->getName().c_str());
           nPlugs = nullptr;
      }
  }
  
  plugs.insert(std::make_pair(key, InjectionPlugConfig(package,name,runInternal,tests, nPlugs)));
}
