...
namespace VnV
{
  ...
  void WorkflowStore::loadSubJob(std::string package, std::string name, const json &config, std::shared_ptr<JobManager> jm)
  {
  ...
  std::shared_ptr<JobManager> WorkflowStore::buildJobManager(std::string mainApplicationName, std::string workflowName,
  ...
  bool WorkflowStore::registeredWorkflow(std::string package, std::string name)
  {
    return true; // We dont know which ones will run.
  }

  void WorkflowStore::addScheduler(std::string packageName, std::string name, const json &schema, scheduler_ptr m)
  {
    scheduler_factory[packageName + ":" + name] = std::make_pair(schema, m);
  }

  void WorkflowStore::addValidator(std::string packageName, std::string name, const json &schema, validator_ptr m)
  {
    validator_factory[packageName + ":" + name] = std::make_pair(schema, m);
  }

  void WorkflowStore::addScriptGenerator(std::string packageName, std::string name, const json &schema, script_gen_ptr m)
  {
    script_factory[packageName + ":" + name] = std::make_pair(schema, m);
  }

  void WorkflowStore::addWorkflow(std::string packageName, std::string name, const json &schema, workflow_ptr m)
  {
    workflow_factory[packageName + ":" + name] = std::make_pair(schema, m);
  }

  json WorkflowStore::schema(json &packageJson)
  {
