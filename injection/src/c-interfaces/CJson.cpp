...
void VnV_CJson_getErrorMessage(int code, char** message) {
  *message = (char*)malloc(100 * sizeof(char));
  ...
  case CJSON_INVALID_DATA:
    strcpy(*message, "data object is null");
    break;
  case CJSON_NOT_AN_ARRAY:
    strcpy(*message, "json object is not an array");
    break;
  case CJSON_NOT_A_INT:
    strcpy(*message, "json object is not an integer");
    break;
  case CJSON_NOT_A_BOOL:
    strcpy(*message, "json object is not a bool");
    break;
  case CJSON_NOT_A_FLOAT:
    strcpy(*message, "json object is not a float");
    break;
  case CJSON_NOT_A_UNSIGNED_INT:
    strcpy(*message, "json object is not an unsigned int");
    break;
  case CJSON_NOT_AN_OBJECT:
    strcpy(*message, "json object is not a object");
    break;
  case CJSON_NOT_A_NUMBER:
    strcpy(*message, "json object is not a number");
    break;
  case CJSON_NOT_A_STRING:
    strcpy(*message, "json object is not a string");
    break;
  case CJSON_OBJECT_KEY_DOES_NOT_EXIST:
    strcpy(*message, "key does not exist in json object");
    break;
  case CJSON_ARRAY_INDEX_OUT_OF_BOUNDS:
    strcpy(*message, "array index out of bounds for json array");
    break;
  }
}
...
json* VnV_CJson_castJson(c_json jsonRaw) { return (json*)jsonRaw.data; }
...
void VnV_CJson_is_null(c_json json, bool* result, int* err) {
  if (VnV_CJson_check(json, err)) *result = VnV_CJson_castJson(json)->is_null();
}
