...
class MPIReq : public IRequest {
 public:
  ...
  MPI_Request* request() { return &req; }
};
...
void MPI_REDUCTION()
  ...
  MPI_Comm comm;
  MPI_Comm* comm_ptr() { return &comm; }

  static int sMap(int source) { return (source < 0) ? MPI_ANY_SOURCE : source; }
  static int tMap(int tag) { return (tag < 0) ? MPI_ANY_TAG : tag; }
  static std::map<DataType, int> dMap = {
      {DataType::Long, MPI_LONG},     {DataType::Int, MPI_INT},
      {DataType::Double, MPI_DOUBLE}, {DataType::Byte, MPI_BYTE},
      {DataType::Char, MPI_CHAR},
  };
  static std::map<int, DataType> idMap = {
      {MPI_LONG, DataType::Long},     {MPI_INT, DataType::Int},
      {MPI_DOUBLE, DataType::Double}, {MPI_BYTE, DataType::Byte},
  ...
  static std::shared_ptr<MPICommunicator> createRange(int start, int end,
                                                      int stride, int tag,
  ...
  static std::shared_ptr<MPICommunicator> createList(std::vector<int>& ranks,
                                                     int tag,
                                                     MPICommunicator* comm) {
    auto s = create();
    MPI_Group group, newGroup;
  ...
  static std::shared_ptr<MPICommunicator> createSplit(int color, int key,
                                                      MPICommunicator* comm) {
    auto s = create();
  ...
  std::string name() { return MPICOMMNAME_S; }

  void init(int* argc, char** argv) override { MPI_Init(&argc, &argc); }
  ...
  VnV::ICommunicator_ptr duplicate() { return createDup(this); }
  VnV::ICommunicator_ptr split() { return createSplit(color, key, this); }
  ...
  VnV::ICommunicator_ptr world() { return createWorld(); }
  VnV::ICommunicator_ptr self() { return createSelf(); }

  int abort(int errorCode) { MPI_Abort(comm, errorCode); }
  ...
  IStatus_ptr wait(IRequest_ptr& request) { return request->wait(); }
  ...
  std::pair<IStatus_vec, int> test_(IRequest_vec& requests, int& flag,
                                    bool any) {
    std::vector<MPI_Request> r(requests.size());
    std::vector<MPI_Status> s(requests.size());
    for (auto it : requests) {
  ...
  int count(IStatus_ptr status) { return status->count(); }

  void send(void* buffer, int size, int dest_, int tag_, DataType dtype,
            MessageType sync) {
    int dest = sMap(dest_);
    int tag = tMap(tag_);
  ...
  IRequest_ptr iSend(void* buffer, int size, int dest, int tag, DataType dtype,
                     MessageType sync) {
    IRequest_ptr ptr = create();
    if (sync == MessageType::sync)
      MPI_Issend(buffer, size, dMap[dtype], sMap(dest), tMap(tag), comm,
                 cast(ptr)->request());
    else if (sync == MessageType::buffered) {
      MPI_Ibsend(buffer, size, dMap[dtype], sMap(dest), tMap(tag), comm,
                 cast(ptr)->request());
    } else {
      MPI_Isend(buffer, size, dMap[dtype], sMap(dest), tMap(tag), comm,
                cast(ptr)->request());
    }
    return ptr;
  }
  ...
  // advance.
  std::tuple<IStatus_ptr, void*, int> recv(int source, int tag,
                                           DataType dtype) {
  ...
  void* scatter(void* data, int count, int root, DataType dtype) {
    ...
    MPI_Scatter(data, count, dMap[dtype], recvbuffer, count, dMap[type], root,
                comm);
    return recvbuffer;
  }
  ...
  void* gather(void* data, int count, int root, DataType dtype) {
    ...
    MPI_Type_size(dMap[dtype], &dsize);
    void* recvbuffer = malloc(count * dsize);
    MPI_Gather(data, count, dMap[dtype], recvbuffer, count, dMap[dtype], root,
               comm);
  }

  void* reduce(void* data, int count, DataType dtype, IReduction_ptr op,
               int root) {
