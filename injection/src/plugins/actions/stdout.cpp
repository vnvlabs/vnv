...
class StdoutAction : public VnV::IAction {
 ...
 public:
  ...
  void injectionPointStart(std::string packageName, std::string id) override { _read(); };

  void injectionPointIteration(std::string stageId) override { _read(); };

  void injectionPointEnd() override { _read(); };
 ...
 *
 */
INJECTION_ACTION(VNVPACKAGENAME, stdout, "{\"type\":\"object\"}") { return new StdoutAction(); }