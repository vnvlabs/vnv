...
public:
     ...
     void Process() {
          ...
          if (sendQueue.size() == 0 || recvQueue.size() == 0 ) { return; }// no sends to process or no recv placed
     ...
     VnV::Communication::ICommunicator_ptr duplicate() { return std::make_shared<SerialCommunicator>();}
     VnV::Communication::ICommunicator_ptr split(int color, int key) {return duplicate();}
     ...
     VnV::Communication::IRequest_ptr ISend(void *buffer, int count, int dest, int tag, int dataTypeSize) {
     ...
     VnV::Communication::IStatus_ptr Recv(void *buffer, int count, int dest, int tag, int dataTypeSize) {
     ...
     VnV::Communication::IRequest_ptr IRecv(void *buffer, int count, int dest, int tag, int dataTypeSize) {
          auto s = getSR(buffer,count*dataTypeSize,tag, false);
          getRecvQueue().push_back(s);
          return s;
     }
     ...
     VnV::Communication::IStatus_vec WaitAll(VnV::Communication::IRequest_vec &vec) {
     ...
     std::pair<VnV::Communication::IStatus_ptr, int> WaitAny(VnV::Communication::IRequest_vec &vec) {
        std::vector<SerialRequest*> r(vec.size());
        std::vector<IStatus_ptr> res(vec.size());
        int count = 0;
        for (auto it : vec ) {
     ...
     std::pair<VnV::Communication::IStatus_ptr, int> Test(VnV::Communication::IRequest_ptr ptr) {
     ...
     std::pair<VnV::Communication::IStatus_vec, int> TestAll(VnV::Communication::IRequest_vec &vec) {
          Process();
          IStatus_vec res;
          int f = 1;
          for (auto it : vec ) {
     ...
     std::tuple<VnV::Communication::IStatus_ptr, int, int> TestAny(VnV::Communication::IRequest_vec &vec) {
          Process();
          int f = 0;
          for (auto it : vec ) {
             SerialRequest* r = (SerialRequest*) it.get();
     ...
     void Reduce(void *buffer, int count, void *recvBuffer, int dataTypeSize, VnV::Communication::OpType op, int root) {
         Gather(buffer,count, recvBuffer, dataTypeSize,0);
     }

     void AllReduce(void *buffer, int count, void *recvBuffer, int dataTypeSize, VnV::Communication::OpType op) {
          Reduce(buffer,count, recvBuffer,dataTypeSize,op,0);
     }

     void Scan(void *buffer, int count, void *recvBuffer, int dataTypeSize, VnV::Communication::OpType op) {
          Reduce(buffer,count, recvBuffer,dataTypeSize,op,0);
     }

     void Abort(int errorcode) {
          VnV::VnVExceptionBase("Serial Abort called" ) ;
     }
};
