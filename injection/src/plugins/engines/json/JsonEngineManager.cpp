...
#define X(type) \
void JsonEngineManager::Put(VnV_Comm /**comm**/,std::string variableName, const type& value) {\
      json j;\
      j["id"] = getId();\
      j["name"] = variableName;\
      j["type"] = #type;\
...
std::string JsonEngineManager::Dump(int d) {
...
JsonEngineManager::JsonEngineManager() {
    ...
    mainJson["info"]["date"] = std::chrono::duration_cast< std::chrono::milliseconds>(
...
void JsonEngineManager::Log(VnV_Comm /**comm**/, const char *package, int stage, std::string level, std::string message) {
...
void JsonEngineManager::injectionPointEndedCallBack(VnV_Comm /**comm**/, std::string id, InjectionPointType type, std::string stageVal) {
...
void JsonEngineManager::injectionPointStartedCallBack(VnV_Comm /**comm**/, std::string id, InjectionPointType type, std::string stageVal) {
    json ip;
    json stage;
...
void JsonEngineManager::testStartedCallBack(VnV_Comm /**comm**/, std::string testName) {
    json j;
    j["id"] = getId();
    j["name"] = testName;
    j["package"] = "TODO";
...
void JsonEngineManager::testFinishedCallBack(VnV_Comm /**comm**/, bool result_) {
    pop(2);
}
