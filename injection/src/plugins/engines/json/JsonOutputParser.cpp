...
template <typename T>
std::shared_ptr<T> mks(const json& json) {
...
std::shared_ptr<ArrayNode> genArrayNode(const json &j, std::string name, std::string id) {
...
std::shared_ptr<MapNode> genMapNode(const json &j, std::string name, std::string id) {
...
std::shared_ptr<UnitTestNode> genUnitTestNode(const json& j) {
     ...
     n->package = j["package"];
     n->children = genArrayNode(j["children"], "children", j["id"].get<std::string>() + "-children");
     n->resultsMap = genMapNode(j["results"],"results",j["id"].get<std::string>() + "-results");
     return n;
}
...
std::shared_ptr<DataBase> nodeDispatcher(const json& j) {
    ...
    if (node ==  "Data" ) {
          ...
          if (type == "double") return genDataNode<DoubleNode,double>(j);
          else if (type == "float") return genDataNode<FloatNode,float>(j);
          else if (type == "std::string")  return genDataNode<StringNode,std::string>(j);
          else if (type == "long") return genDataNode<LongNode,long>(j);
          else if (type == "bool") return genDataNode<BoolNode,bool>(j);
          else if (type == "int") return genDataNode<IntegerNode,int>(j);
          else if (type == "json") return genJsonNode(j);
          else throw VnVExceptionBase("Unknown Data node type");
    }
    else if (node == "Log")  return genLogNode(j);
    else if (node == "Info")  return genInfoNode(j);
