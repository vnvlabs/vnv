...
template <class T> std::vector<T> StreamPatch::GetCommon(Variable<T>& variable) {
  try {
...
StreamPatch::StreamPatch(std::string name) : m_Name(name), m_ADIOS(), m_IO(m_ADIOS.DeclareIO(name)) { CheckOpen(); }

#if ADIOS2_USE_MPI
StreamPatch::StreamPatch(const std::string& name, MPI_Comm comm) : m_ADIOS(comm), m_IO(m_ADIOS.DeclareIO(name)) {
  CheckOpen();
}
...
#define declare_template_instantiation(T) \
                                          \
  template std::vector<T> StreamPatch::Read<T>(const std::string&, const size_t);
...
#if ADIOS2_USE_MPI
ifstream::ifstream(const std::string& name, MPI_Comm comm)
