...
class JsonFileStream : public FileStream<JsonFileIterator,json> {
 ...
 std::string createResponseDirectory() {
   if (response_stub.length() == 0 ) {
      response_stub = VnV::DistUtils::join({filestub,"__response__"},0777,true);
  ...
  virtual bool fetch(long id, const json& schema, long timeoutInSeconds, json& response ) {
     


     // Get a unique id for this request. 
     std::string c = std::to_string(++INIDCOUNT);
     std::string fname = VnV::DistUtils::join({createResponseDirectory(), c + "_" + std::to_string(id) }, 0777,false);
     
     // Open a file called <fname>.writing. 
     std::ofstream f(fname + ".writing");
     
     // Write the schema provided and the expiry time to the file.
     long expiry = std::chrono::duration_cast<std::chrono::seconds>(std::chrono::system_clock::now().time_since_epoch()).count() + timeoutInSeconds;
     json j = json::object();
     j["schema"] = schema;
     j["expires"] = expiry;
     f << j.dump(4);
     ...
     // Rename the file the <fname>.pending. This makes sure the reader does not see 
     // the file until we are done writing to it. 
  ...
  virtual void write(long id, const json& obj, long jid) override {
    ...
    if (it != streams.end()) {
      if (it->second.good()) {
        // Writing to a closed stream does NOT throw an exception (by default). So, we check for "good" first/ 
