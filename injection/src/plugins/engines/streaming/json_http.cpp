...
MHD_Result answer_to_connection(void* cls, struct MHD_Connection* connection,
  ...
  const char* page = "OK";
  std::map<long, VnV::StreamReader::JsonSingleStream>* data =
      (std::map<long, VnV::StreamReader::JsonSingleStream>*)cls;
  ...
  if (0 == strcmp(method, MHD_HTTP_METHOD_POST)) {
    ...
    auto it = data->find(request->getStreamId());
    if (it == data->end()) {
      VnV::StreamReader::JsonSingleStream jentry;
      VnV::StreamReader::JsonElement e(request->getId(), request->getData());
      jentry.add(request->getId(), request->getData());
      data->insert(std::make_pair(request->getStreamId(), jentry));
    } else {
      it->second.add(request->getId(), request->getData());
    }
    delete request;
  ...
  bool start_stream_reader() override {
    if (daemon != NULL) {
      daemon = MHD_start_daemon(MHD_USE_INTERNAL_POLLING_THREAD, port, NULL,
                                NULL, &answer_to_connection, &instreams, NULL,
                                MHD_OPTION_END);
    }
    return daemon != NULL;
  }
...
class JsonHttpStream : public StreamWriter<json> {
 ...
 public:
  ...
  virtual std::shared_ptr<IRootNode> parse(std::string file,
                                           long& id) override {
    auto stream =
