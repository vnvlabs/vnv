...
void XmlParser::addTest(test_p t, std::vector<TestConfig> &testConfigs, std::set<std::string> &runScopes) {
...
void XmlParser::addInjectionPoint(injectionPoint_p ip, std::set<std::string> &runScopes, std::map<std::string,std::vector<TestConfig>> &ips) {
...
void XmlParser::addTestLibrary(testLibrary_p lib, std::set<std::string> &libs) {
    int i = 0;   
...
EngineInfo XmlParser::getEngineInfo(outputEngine_p e) {
    EngineInfo info;
...
RunInfo XmlParser::parseXMLFile(vv::Document *doc ) {
...
RunInfo XmlParser::parse(std::string filename) {
...
extern "C" {
 IParser*  getXmlParser(std::string what) {
   return new XmlParser();
 }
}

class XmlParser_proxy {                                                                               
public:                                                                                                       
  XmlParser_proxy(){                                                                                  
    VnV_registerParser("xml",getXmlParser);                
  }                                                                                                           
};                                                                                                            
                                                                                                            
XmlParser_proxy xmlParser_proxy; 
