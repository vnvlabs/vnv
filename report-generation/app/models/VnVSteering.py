...
class VnVSteering:
    ...
    def list_pending(self):
        pending = glob.glob(os.path.join(self.filename,"__response__","*.pending"))
        pend = []
        for i in pending:
            f = open(i,'r')
            message = json.load(f)
            ...
            if int(time.time()) > expiry:
               ...
               os.remove(i)
                ...
                pend.append(SteerRender(self.id_, i[len(os.path.join(self.filename,"__response__"))+1:-8], message["schema"], expiry))
            f.close()

        return pend;

    def render(self,id_):
        return self.get(os.path.join(self.filename,"__response__",id_ + ".pending"))

    def get(self,filename):
        with open(filename, 'r') as f:
            message = json.load(f)
            expiry = message["expires"]
            return SteerRender(self.id_, filename[len(os.path.join(self.filename, "__response__")) + 1:-8], message["schema"], expiry)
