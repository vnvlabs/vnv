...
def generateCppFile(name, parameters, includes, sys_includes) :
    ...
    s += "\t %s(VVTestConfig config) : VV::BasicTest(config) {\n" %name
...
def generateMarkdownFile(name) :    
    return mdComment("This is the Markdown file for %s " % name )  
...
print generateMarkdownFile(args['name'])
